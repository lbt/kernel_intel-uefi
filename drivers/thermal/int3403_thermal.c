...
static int sys_set_trip_temp(struct thermal_zone_device *tzone, int trip,
							unsigned long temp)
{
	struct acpi_device *device = tzone->devdata;
	acpi_status status;
	...
	struct acpi_object_list arg_list = { .count = 1, .pointer = &arg0, };
	acpi_handle handle;

	snprintf(name, sizeof(name), "PAT%d", trip);
	status = acpi_get_handle(device->handle, name, &handle);
	if (ACPI_SUCCESS(status)) {
		arg0.integer.value = MILLI_CELSIUS_TO_DECI_KELVIN(temp,
					KELVIN_OFFSET);
